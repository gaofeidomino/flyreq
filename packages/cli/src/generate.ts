...
export function generateResourceFile(resourceName: string, endpoints: Record<string, EndpointDef>): string {
  ...
  if (flags.idempotent) coreImports.push('createIdempotentRequestor')
  if (flags.cache) coreImports.push('createCacheRequestor')
  if (flags.plain) coreImports.push('useRequestor')

  const lines: string[] = [
    `/* Generated by @flyreq/cli — do not edit; put customizations in overrides/ */`,
    `import { ${coreImports.join(', ')} } from '@flyreq/core'`,
    `import { busCall } from '@flyreq/bus'`,
    '',
  ]
