/**
 * 场景：DIP 接线（高级）—— 换传输不必改业务代码
 *
 * 日常请看 `quick`：setupFlyreq + flyreq.get。
 * request-core 只依赖 Requestor 接口。换实现 = 换注入。
 */
import {
  bootstrapRequestor,
  flyreq,
...
export async function runSetup(): Promise<void> {
  heading('1. setupFlyreq()：注册实现后一键注入')
  resetRequestor()
  resetFlyreqClient()
  ...
  const ping = await flyreq.get<{ url: string }>('/api/ping')
